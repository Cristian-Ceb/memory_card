...
questions = [
    Question('Сколько сегодня градусов?', '4', '-10', '43905830450390940', '-6590645745445'),
    Question('Вы считаете, что вы могли бы быть хорошим бизнесменом?', 'Я считаю на пальцах', 'Да', 'Нет', 'Не знаю'),
    Question('Кот Шрюдингера выжил?', 'Я чё знаю', 'Да', 'Нет', 'Шансы равны, как на выживание, так и на смерть (душнила)'),
    Question('Обед?', 'Уютненько', 'Ужин', 'Завтрак', 'Полдник'),
]
...
def get_procent(score):
    ...
    result += 'Процент верных ответов: ' + str(procent) + ', делайте выводы ачё'
    return result
...
def show_result():
    if is_all_checked():
        if buttons[0].isChecked():
            ...
            lbl_result.setText('Бро, тебе нужно тренироваться, ты ответил неправильно')
        grpbox_answers.hide()
        grpbox_result.show()
